...
registers a Numba ``@cfunc`` for that loss subclass; the dispatcher in
:mod:`riesztree.grow` then routes continuous splits (``splitter='exact'``)
through the user's compiled kernel at C speed.
"""
...
# Loss-kind constants exported from the compiled extension.
...
# ---------------------------------------------------------------------------
# User-loss registry: subclass-of-Loss → (leaf_loss_cfunc, alpha_at_opt).
#
# The cfunc's ``.address`` feeds the Cython splitter's tight loop; the cfunc
# itself (numba cfuncs are Python-callable) and ``alpha_at_opt`` serve the
# Python paths (``_make_leaf``, pruning, holdout loss).

_USER_LOSS_REGISTRY: dict[type, tuple[callable, callable]] = {}
...
def register_fast_leaf_solver(
    ...
    leaf_loss_cfunc
        A Numba ``@cfunc("float64(float64, float64)")`` computing the
        leaf loss at its optimum from the leaf sums ``(D, C)``. The
        splitter calls its ``.address`` without the GIL; the Python paths
        call the cfunc directly.
    alpha_at_opt
    ...
    Registered losses work with ``splitter='exact'``.
    ...
    """
    addr = int(getattr(leaf_loss_cfunc, "address", 0) or 0)
    if addr <= 0 or not callable(leaf_loss_cfunc):
        raise TypeError(
            f"register_fast_leaf_solver: leaf_loss_cfunc for "
            f"{loss_class.__name__} must be a numba cfunc (callable, with a "
            f"positive .address); got {type(leaf_loss_cfunc).__name__}."
        )
    ...
    _USER_LOSS_REGISTRY[loss_class] = (leaf_loss_cfunc, alpha_at_opt)


def _lookup_user_kernel(loss: Loss) -> tuple[callable, callable] | None:
    """Return ``(leaf_loss_cfunc, alpha_at_opt)`` for the closest
    registered ancestor of ``type(loss)``, or ``None``."""
...
def loss_kind_for(loss: Loss) -> tuple[int, float, float, int]:
    """Map a Loss to ``(loss_kind, bounded_lo, bounded_hi, user_addr)``.

    For built-ins ``user_addr`` is 0; for user-registered losses
    ``loss_kind`` is :data:`LOSS_USER_CFUNC` and ``user_addr`` is the
    registered C-callable address (the other slots are unused). Callers
    run :func:`riesztree.splitter.make_leaf_solvers` first, which rejects
    any other loss.
    ...
    if user_entry is not None:
        return LOSS_USER_CFUNC, np.nan, np.nan, int(user_entry[0].address)
