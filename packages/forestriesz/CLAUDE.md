...
This package depends on `rieszreg` for the shared abstractions (`Estimand`, `Loss`, `Backend` / `MomentBackend` Protocols, `Diagnostics`, `RieszEstimator` orchestrator, `trace`, `AugmentedDataset`) and on `riesztree` for the per-tree augmented learner. See [`DESIGN.md`](../../DESIGN.md) at the repo root for the meta-package design and the contract every implementation package follows. `forestriesz` contributes:
