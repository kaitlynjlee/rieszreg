...
- **Backends**: `XGBoostBackend(hessian_floor="auto", gradient_only=False)` and `SklearnBackend(base_learner_factory)`. Both register their `Predictor` loader for the registry-based save/load path on import.
...
- `_make_objective` floors the Hessian. `hessian_floor="auto"` (default) floors each row at `loss.curvature_eta(η̂)` = `h''(α̂)·(dα/dη)²`, the curvature an observed row has at the current prediction. For `SquaredLoss` that is the constant 2, identical to the older fixed `hessian_floor=2.0`. A fixed 2.0 under-stepped the other losses (Bernoulli's true curvature is ≤ 0.25, KL's is α̂). In a simulation (n=2000, 5 seeds, α-RMSE vs truth), `"auto"` vs `2.0`: KL/TSM 0.45 vs 1.52 at 200 trees (0.45 vs 0.67 with early stopping); BoundedSquared(0,1) 0.09 vs 0.13; Bernoulli 0.085 vs 0.089 with early stopping (in 50 vs 374 trees), but 0.101 vs 0.094 at a fixed 200 trees, because the larger steps overshoot without early stopping. Earlier we used `eps=1e-6`, which made counterfactual leaves degenerate in xgboost's leaf-weight Newton step (`-G/(H+λ)` with H≈0) and required `reg_lambda=100` to stabilize. The new floor mimics the row-uniform weighting that first-order gradient boosting (Friedman 2001) uses by construction; standard xgboost-style hyperparameters now work without sledgehammer regularization.
