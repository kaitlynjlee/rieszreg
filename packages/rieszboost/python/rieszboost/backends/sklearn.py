    ...
    def fit_augmented(
        ...
        *,
        ...
        for it in range(self.n_estimators):
            ...
            if getattr(learner, "get_params", None) and learner.get_params().get("random_state", 0) is None:
                learner.set_params(random_state=int(seeds[it]))
            learner.fit(aug_train.features, -grad_train)
            h_train = np.asarray(learner.predict(aug_train.features))

            gamma = _line_search(
                loss, is_original, potential_deriv_coef, F_train, grad_train, h_train
            )
            ...
            if have_valid:
                ...
                val_loss = aug_valid.mean_loss(loss, loss.link_to_alpha(F_val))
        ...
        # Like xgboost, truncate to the best round only under early stopping;
        # otherwise every fitted learner is kept.
