...
Leave `covariates` out to use every column of `Z` other than the treatment: `ATE(treatment="treat")` fits $\alpha$ on `treat` plus all eight covariates here. Keep the outcome out of `Z` when you do this. After fitting, `est.feature_names_in_` lists the columns $\alpha$ uses.
