...
|---|---|---|
| `ATE(treatment, covariates)` | $\mu(1, x) - \mu(0, x)$ | Average treatment effect — see [`rieszboost/examples/lee_schuler/binary_dgp.py`](https://github.com/rieszreg/rieszreg/blob/main/packages/rieszboost/examples/lee_schuler/binary_dgp.py). |
...
| `TSM(level, treatment, covariates)` | $\mu(\text{level}, x)$ | Treatment-specific mean — see [`rieszboost/examples/tsm.py`](https://github.com/rieszreg/rieszreg/blob/main/packages/rieszboost/examples/tsm.py). |
| `AdditiveShift(delta, ...)` | $\mu(a + \delta, x) - \mu(a, x)$ | Continuous-treatment shift effect — see [`rieszboost/examples/lee_schuler/continuous_dgp.py`](https://github.com/rieszreg/rieszreg/blob/main/packages/rieszboost/examples/lee_schuler/continuous_dgp.py). |
...
Leave `covariates` out to use every column of `Z` other than the treatment: `ATE(treatment="treat")` fits $\alpha$ on `treat` plus all eight covariates here. Keep the outcome out of `Z` when you do this. After fitting, `est.estimand_.feature_keys` lists the columns $\alpha$ uses, in the order an ndarray passed to `predict` is read.

When `covariates` is given, `Z` must contain every name in `feature_keys`, and other columns are ignored. With an `ndarray`, the treatment is column 0 and the rest are the covariates, in order.
...
$\bar m$ is the mean of the true α. When it lies outside the loss's α-range (for example $\bar m = 1$ for `TSM` under `BernoulliLoss`), no α̂ in that range can be right, and `fit` raises. See [Losses](losses.qmd).
:::
