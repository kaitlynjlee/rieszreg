...
**Start with `RieszBooster` at its default settings.** For $n$ in the low thousands to tens of thousands of rows it is a good first choice. In a cross-fit ATE simulation with the defaults, the one-step estimate had bias of about $-0.001$ and 95% intervals covered the truth at least 95% of the time. Reach for the others when your problem fits their strengths:
...
- **[A single decision tree](backends/tree.qmd)** — `riesztree` fits one tree. It is fast and easy to read: each leaf holds one weight for a subgroup.

Rules of thumb:

- Small $n$ (hundreds to a few thousand) or a smooth, low-dimensional problem: try kernel ridge (`krrr`).
- Large $n$ or many covariates (including image or text features): try the neural net (`riesznet`).
- Confidence intervals on $\alpha(z)$ itself: use the forest (`forestriesz.ForestRieszRegressor` with `honest=True, inference=True`).
- A readable rule you can inspect: use the single tree (`riesztree`).

The five learner families produce different $\hat\alpha$, but all are minimizers of the same population objective and converge to $\alpha$ under their respective consistency conditions.
