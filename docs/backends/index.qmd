...
`MomentBackend.fit_rows(X_train, X_valid, estimand, loss, …)` is for learners whose loss decomposes per original row. You receive the original-row feature arrays (columns in `estimand.feature_keys` order) plus the estimand. `estimand.augment(X, ys)` gives every evaluation point $a_j$ with coefficient $c_j = -C_r$ and the original row it came from (`origin_index`), so each row's moment is one vectorised group-by. Each row is a single training example to the learner. Reference learners using this entry point: random forests (`ForestRieszBackend`) and neural nets (`TorchBackend`).
...
class MyMomentBackend:
    ...
    def fit_rows(self, X_train, X_valid, estimand, loss,
        ...
        aug = estimand.augment(X_train, ys=ys_train)
        points, coefs, row = aug.features, -aug.potential_deriv_coef, aug.origin_index
