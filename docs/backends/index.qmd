...
A backend fits the Riesz representer for a given estimand and loss. Five implementation packages ship today; each slots into `RieszEstimator` through one of two Protocols. `rieszboost` and `krrr` consume the augmented dataset of evaluation points with weights $(D_r, C_r)$ (`Backend.fit_augmented`). `forestriesz` ships *both*: an augmentation-style `AugForestRieszRegressor` that works on every estimand without per-estimand configuration (`Backend.fit_augmented`), and a moment-style `ForestRieszRegressor` that consumes original rows plus a user-supplied list of basis functions (`MomentBackend.fit_rows`) and supports honest-split confidence intervals. `riesznet` consumes the augmented dataset through `Backend.fit_augmented`, grouping it by original row to train a PyTorch model with autograd on minibatches of rows.
...
| **Scaling** | Linear in $n$ via xgboost | $O(n^3)$ direct; Nyström + RFF for larger $n$ | $O(n \log n)$ per tree | Per-epoch cost is one forward pass over $n + \sum_i k_i$ points; scales to GPU |
| **Confidence intervals** | No | No | Yes — `predict_interval` on single-basis fits (moment-style only) | No |
| **Convenience class** | `rieszboost.RieszBooster` | `krrr.KernelRieszRegressor` | `forestriesz.AugForestRieszRegressor` (recommended; works on every estimand) and `forestriesz.ForestRieszRegressor` (the published ForestRiesz method, kept for comparison; supports CIs) | `riesznet.RieszNet` (default MLP); `riesznet.TorchBackend` for arbitrary `nn.Module` factories |

A fifth learner, [`riesztree`](tree.qmd), fits a single decision tree (`RieszTreeRegressor`). It is fast and easy to inspect, and it supports all four losses.

### Where to start

Start with `RieszBooster` at its default settings. For $n$ in the low thousands to tens of thousands of rows it is a good first choice. In a cross-fit ATE simulation with the defaults, the one-step estimate had bias of about $-0.001$ and 95% intervals covered the truth at least 95% of the time.

Switch when your problem fits another learner's strengths:

- **Kernel ridge (`krrr`)** for small $n$ or a smooth, low-dimensional $\alpha$.
- **Neural net (`riesznet`)** for large $n$, many covariates, or image and text features.
- **Forest (`forestriesz`)** when you need confidence intervals on $\alpha(z)$ (`ForestRieszRegressor` with `honest=True, inference=True`).
- **Single tree (`riesztree`)** when you want a rule you can read.

### The same setting under different names

Each learner names its settings after the library it follows (xgboost, sklearn, PyTorch). This table lines them up.

| Setting | `RieszBooster` | `KernelRieszRegressor` | `AugForestRieszRegressor` | `RieszTreeRegressor` | `RieszNet` |
|---|---|---|---|---|---|
| Amount of fitting | `n_estimators` (trees) | — | `n_estimators` (trees) | `max_depth`, `max_leaf_nodes` | `epochs` |
| Step size | `learning_rate` | — | — | — | `learning_rate` |
| Regularization | `reg_lambda`, `max_depth` | `lambda_grid` (chosen automatically) | `min_samples_leaf`, `max_depth` | `min_samples_leaf`, `ccp_alpha` | `weight_decay`, `dropout` |
| Early stopping | `early_stopping_rounds` | — | — | `early_stopping_rounds` | `early_stopping_rounds` |
| Held-out fraction | `validation_fraction=0.1` | `validation_fraction=0.2` | — | `validation_fraction=0.1` | `validation_fraction=0.1` |

`validation_fraction` is used only when early stopping is on. `KernelRieszRegressor` always holds out rows, because it picks λ from `lambda_grid` by held-out loss. Passing `eval_set=` to `fit` replaces the internal split in every learner.
...
# Neural network (augmentation-style backend; fit_augmented path)
est = RieszEstimator(
    estimand=ATE(),
...
The convenience classes are thin subclasses that bake in the backend and surface backend-specific hyperparameters as constructor args, so that they fit cleanly into `GridSearchCV`. See the [boosting](boosting.qmd), [kernel](kernel.qmd), [forest](forest.qmd), [tree](tree.qmd), and [neural](neural.qmd) pages for each.
...
`Backend.fit_augmented(aug_train, aug_valid, loss, …)` is for learners whose loss decomposes naturally over augmented evaluation points. The orchestrator hands you an `AugmentedDataset` of pseudo-rows with `is_original` and `potential_deriv_coef` columns. The empirical Bregman-Riesz loss at $\alpha$ is the sum over augmented rows $r$ of $D_r\,\tilde h(\alpha(Z_r)) + C_r\,h'(\alpha(Z_r))$, where $D_r$ is `is_original` and $C_r$ is `potential_deriv_coef`. Reference learners using this entry point: kernel ridge (`KernelRidgeBackend`), gradient boosting (`XGBoostBackend`, `SklearnBackend`), random forests (`AugForestRieszBackend`), and neural nets (`TorchBackend`, which groups the augmented rows by `origin_index` to minibatch over original rows).

`MomentBackend.fit_rows(X_train, X_valid, estimand, loss, *, aug_train, aug_valid, …)` is for learners that fit on the original rows and need the estimand itself. You receive the original-row feature arrays (columns in `estimand.feature_keys` order), the estimand, and the same augmented datasets `fit_augmented` would get. The augmented rows give every evaluation point $a_j$ with coefficient $c_j = -C_r$ and the original row it came from (`origin_index`), so each row's moment is one vectorised group-by. Each row is a single training example to the learner. Reference learner using this entry point: the EconML-based random forest (`ForestRieszBackend`), which also reads the estimand to pick its basis functions.

Pick `fit_augmented` unless your learner needs the original-row matrix or the estimand: the augmented dataset already carries each row's moment through `origin_index`. The orchestrator dispatches to the moment path only when `fit_rows` is the only method present.
...
class MyMomentBackend:
    ...
    def fit_rows(self, X_train, X_valid, estimand, loss,
                 *, aug_train, aug_valid, base_score, random_state):
        # m(α)(Z_i, Y_i) = sum_j c_j · α(a_j). The augmented rows carry every
        # point a_j, its coefficient c_j = -C_r, and the row i it came from.
        points = aug_train.features
        coefs, row = -aug_train.potential_deriv_coef, aug_train.origin_index

        params = my_moment_fit(
            X_train, points, coefs, row,
