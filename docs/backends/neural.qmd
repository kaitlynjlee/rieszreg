...
The backend implements `Backend.fit_augmented` and groups the augmented rows by original row: for each original row `z_i`,

$$
L_i = \tilde h(\alpha(z_i)) - \sum_{j} c_j \cdot h'(\alpha(p_j))
$$

where the `(c_j, p_j)` pairs are the evaluation points of `m` at `z_i` and the model produces α via a Bregman link. These points come from the augmented dataset the orchestrator builds with `estimand.augment`, so the built-in estimands need no per-row Python work. Each minibatch evaluates the network only at its own rows' points. The training loop minimizes the mean of `L_i` with autograd.
...
| Knob | Default | Notes |
|---|---|---|
| `module_factory` | placeholder | `Callable[[int], nn.Module]` returning a model with scalar output. The default placeholder raises at `fit`, so set it when you build `TorchBackend` yourself. `RieszNet` fills in an MLP (`riesznet.modules.build_mlp`). |
| `optimizer_factory` | placeholder | `Callable[[Iterable[Parameter]], Optimizer]`. The default placeholder raises at `fit`. `RieszNet` fills in Adam (`riesznet.modules.build_adam`). |
...
| `BernoulliLoss` | ✅ | sigmoid | Forces α̂ ∈ (0, 1), so the true Riesz representer must lie in (0, 1). No built-in estimand satisfies this. Use it for custom estimands whose representer does. |
