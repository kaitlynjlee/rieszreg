...
print(f"depthwise leaves: {est_dw.get_n_leaves()}")
print(f"leafwise  leaves: {est_lw.get_n_leaves()}")
```
...
- **Early stopping** via `early_stopping_rounds` + `validation_fraction`. Stops growing when the held-out augmented loss has not improved for that many consecutive accepted splits, mirroring `rieszboost` / `riesznet`. The fitted tree is then rolled back to the partial tree with the best held-out loss.
...
print(f"unpruned: {RieszTreeRegressor(estimand=ATE(treatment='a', covariates=('x',)), max_depth=10).fit(df).get_n_leaves()} leaves")
print(f"pruned  : {est_pruned.get_n_leaves()} leaves")
print(f"early-stop: {est_es.get_n_leaves()} leaves")
```
...
| `early_stopping_rounds` | None | Stop when held-out augmented loss has not improved for that many accepted splits, then roll back to the best partial tree. |
...
| `random_state` | 0 | Seeds the per-split feature subsample (`max_features`), the per-feature random threshold (`splitter='random'`) and the histogram bin subsample. |
| `splitter` | `"exact"` | One of `"exact"` (Cython per-feature sweep), `"hist"` (Cython quantile-binned histogram, fastest at large $n$), or `"random"` (sklearn ExtraTrees-style). |
...
for sp in ["exact", "hist", "random"]:
    ...
    print(f"splitter={sp:6s}  fit={fit_ms:6.1f} ms  leaves={est.get_n_leaves()}")
```

- `"exact"` (default): per-feature linear scan over distinct values. Best partition.
...
For losses outside the four built-ins (`SquaredLoss`, `KLLoss`, `BernoulliLoss`, `BoundedSquaredLoss`), register a Numba `@cfunc` via `riesztree.fast.register_fast_leaf_solver`. Registered losses run on `splitter="exact"`. The registry is checked before the built-ins, so registering a subclass of a built-in loss overrides its kernel for both split search and leaf values. Unregistered custom losses raise `NotImplementedError` at fit.
...
The cfunc is called from the Cython splitter's tight loop without the GIL — same per-evaluation cost as the four built-in losses. See [riesztree's BENCH_BASELINE.md](https://github.com/rieszreg/rieszreg/blob/main/packages/riesztree/BENCH_BASELINE.md) for the locked baselines and a comparison vs sklearn / LightGBM / XGBoost.
...
Out-of-domain children are disqualified (`+∞` leaf loss). `fit` rejects `KLLoss` / `BernoulliLoss` with estimands whose α takes negative values (`ATE`, `ATT`, shifts); see [Losses](../losses.qmd).
...
- **Loss-domain disqualifications.** Under KL / Bernoulli some split candidates put a child outside the link's $\alpha$-domain and are ruled out, so the tree may be shallower than `max_depth`. This is the splitter respecting the domain, not a bug.
