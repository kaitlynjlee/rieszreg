...
# or, for the single-tree backend:
use_python_riesztree("/path/to/RieszReg/.venv/bin/python")
...
| `OutcomeRegNormSq(covariates)` | `rieszreg.OutcomeRegNormSq(covariates)` |
...
| &nbsp;&nbsp;`$diagnose(Z, y=)` | `.diagnose(Z, y=)` (returns a list with `$summary` as a string) |
| &nbsp;&nbsp;`$save(path)` | `.save(path)` |
...
| `Tensor(a, cols_a, b, cols_b)` | `krrr.Tensor(...)` (R column positions are 1-based) |
...
The R wrapper exposes the moment-style `ForestRieszRegressor`. It handles ATE, ATT and TSM through the default treatment-indicator basis. The augmentation-style `AugForestRieszRegressor` is Python-only in v1; call it from R via reticulate when you need a fully estimand-agnostic forest fit.

## API surface (riesztree)

| R | Python equivalent |
|---|---|
| `RieszTreeRegressor$new(estimand, loss, max_depth, min_samples_split, min_samples_leaf, max_leaf_nodes, growth_policy, ccp_alpha, early_stopping_rounds, splitter, ...)` | `riesztree.RieszTreeRegressor(...)` |
| &nbsp;&nbsp;`$fit(df, eval_set=)` | `.fit(Z, eval_set=)` |
| &nbsp;&nbsp;`$predict(df)` | `.predict(Z)` |
| &nbsp;&nbsp;`$score(df)` | `.score(Z)` |
| &nbsp;&nbsp;`$diagnose(df)` | `.diagnose(Z)` |
| &nbsp;&nbsp;`$save(path)` | `.save(path)` |
| `load_riesz_tree_regressor(path)` | `RieszTreeRegressor.load(path)` |

`categorical_features` takes 1-based R column positions.
...
R6's `$method()` syntax maps to Python's `.method()`. The wrappers convert integer-typed arguments (`n_estimators`, `max_depth`, `random_state`, `epochs`, and so on) with `as.integer()`, so `n_estimators = 200` and `n_estimators = 200L` both work. Other arguments are converted by reticulate.

Data frames are converted to pandas column by column. Numeric and logical columns pass through as numbers. Factor columns are converted through their labels, so a treatment coded `factor(c("0", "1"))` arrives as 0/1. ATE and ATT need the treatment coded 0/1 and stop with an error otherwise.
