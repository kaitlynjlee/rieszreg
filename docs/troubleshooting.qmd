...
When `RieszEstimator.fit()` runs and detects both `torch` and `xgboost` already loaded, it emits a one-time `RuntimeWarning` describing the situation, unless `OMP_NUM_THREADS=1` is already set.
