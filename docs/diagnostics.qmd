...
`.diagnose(Z)` works on every learner. Some learners add their own fields on top of the shared ones: `KernelRieszRegressor` reports the chosen λ, support size, effective degrees of freedom, and condition number. `RieszTreeRegressor` and `ForestRieszRegressor` report leaf counts, leaf sizes, and feature importances.

This page walks through `.diagnose()` on an under-tuned booster (so the warnings fire) and maps each warning to the hyperparameter that fixes it. See [Boosting backend](backends/boosting.qmd) and [Kernel backend](backends/kernel.qmd) for the backend-specific tuning recipes.
...
1. *"X% of rows have $|\hat\alpha| >$ threshold — possible near-positivity violation."* Either the data has genuine near-positivity (truncate `extreme_threshold` and report the loss of overlap), or the estimator is over-fitting (tune as below).
2. *"max $|\hat\alpha|$ is >10× the 99th percentile — likely a single extrapolation outlier."* The learner found a region with no nearby training data. Tighten model complexity, increase regularization, or stop earlier.
